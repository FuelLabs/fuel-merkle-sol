...
describe('Sparse Merkle Tree', async () => {
	it('Update and delete', async () => {
		...
		const rootAfterLeaves =
			'0xdc0537167454509d360e0807b673b0bdfde730dd8ce944a43e397e3a16ac322b';
		const rootAfterUpdateExisting =
			'0x846fb76ccb1cd6f3a2802c658a6ab1befd658e25ff7a81e955e14da50fa77c02';
		const rootAfterUpdateNew =
			'0x97405008d58748206f15c393d58c94c94dcc58ed59c3cbec1f0faf58df27634b';
		...
		expect(smt.root).to.equal(rootAfterLeaves);

		// Update an existing leaf to a new value
		smt.update(hash(uintToBytes32(n / 10)), newData);
		expect(smt.root).to.equal(rootAfterUpdateExisting);
		...
		expect(smt.root).to.equal(rootAfterUpdateNew);
